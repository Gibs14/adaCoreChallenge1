...
struct TenantView: View {
    ...
    func getMenuView(for tenant: Tenant) -> some View {
        ...
        case "Nasi Goreng Bang Ali":
            MenuViewA(tenantName: tenant.name)
        case "Kantin Jempol":
            MenuViewB(tenantName: tenant.name)
        case "Zhi Lin":
            MenuViewC(tenantName: tenant.name)
        case "Mama Rosa":
            MenuViewD(tenantName: tenant.name)
        case "Trobeki Juice":
            MenuViewE(tenantName: tenant.name)
        case "Bang Chili Oil":
            MenuViewF(tenantName: tenant.name)
        case "Sari Merah":
            MenuViewG(tenantName: tenant.name)
        default:
            Text("")
        }
    }
